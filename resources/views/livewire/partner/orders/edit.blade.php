                            ...
                            @endphp
                                <div class="row g-5 my-1">
                                    <div class="col-md-3 fv-row fv-plugins-icon-container">
                                        ...
                                        <div id="item_{{$key}}" wire:ignore></div>
...
<script >
    Livewire.on('newSelect2', function (rows) {
        setTimeout(() => {
            multiSelect(rows-1);
        }, 500); //0.5 sec
    });
    var pricing_items = <?=$pricing_items; ?>;
    var rows = <?=json_encode($items); ?>;
    ...
    function multiSelect(index){
        ...
        $('#item_'+index).on('change', function (e) {
            @this.set('items.'+index+'.item_id', $(this).val());
            @this.call('getPrice', index);
        });
    }
