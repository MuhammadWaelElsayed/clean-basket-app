...
class WorkingHours extends Component
{
    ...
    {
        abort_unless(auth()->user()->can('list_working_hours'), 403);
        $this->vendors = Vendor::select('id', 'business_name')->get();
    ...
    {
        $query = VendorWorkingHours::with(['vendor' => function ($q) {
        ...
        $this->workingHours = $query->orderBy('vendor_id')
            ->orderBy('day_of_week')
            ->get();

        return view('livewire.admin.vendors.working-hours', [
    ...
    {
        if (!$this->vendor_id) {
            $this->dispatch('error', 'الرجاء اختيار مغسلة أولاً');
            return;
        }
    ...
    public function showEditForm($id)
    {
        ...
        $this->editingId = $id;
        $this->vendor_id = $workingHour->vendor_id;
